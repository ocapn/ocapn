...
"pipelined" to [promises](#promises), eliminating the need for unnecessary 
round-trips via [Promise Pipelining](#promise-pipelining).
...
When we gain access to a remote promise, such as through an 
[`op:deliver`](#opdeliver) message, it is often desirable to use the result. 
One way to do this would be to wait until the promise has been settled and then 
send the next message to the result. While this approach can be taken within 
OCapN, promise pipelining is preferred.

When an [`op:deliver`](#opdeliver) message is sent with an `answer-pos` specified, 
the promise on the remote session can then be referenced and messaged with the
...
Promise pipelining works whether we receive the promise reference through an 
[`op:deliver`](#opdeliver), [`op:index`](#opindex), [`op:untag`](#opuntag), or
[`op:get`](#opget) message OR as an argument in [`op:deliver`](#opdeliver) args.
...
We set the `listen-mode` in the first two [`op:listen`](#oplisten) messages to 
`shortening-only` because we won't need the peer to send us a reference to the 
car factory or to the car. (At least a shortening listener is always needed.) 
We do set a `listen-mode` of `shortening or settling` for the third message so 
the peer sends us the driving noise.

The messages delivered are as follows:

```
<op:deliver <desc:export 5> ['make-car-factory] 3>
<op:listen <desc:answer 3> <desc:import-object 15> 1>   ; shortening-only listen
<op:deliver <desc:answer 3> ['make-car] 4>
<op:listen <desc:answer 4> <desc:import-object 16> 1>   ; shortening-only listen
<op:deliver <desc:answer 4> ['drive] 5>
<op:listen <desc:answer 5> <desc:import-object 17> 3>   ; shortening or settling listen
...
### `answer-pos`
When a result is desired from the `op:deliver` or 
[promise pipelining](#promise-pipelining) is being enabled, this value 
...
If a non-false `answer-pos` is provided, then the `op:deliver` MUST be followed
up by an [`op:listen`](#oplisten) in either shortening-only or 
shortening-or-settling mode to request notification of any promise shortening
and, in the shortening-or-settling case, the settling of the promise.

### Receiving
The message should be delivered to the object referenced by `to-desc` with the
arguments specified in `args`.

If `answer-pos` is a positive integer, then a promise MUST be created and 
exported at the answer position specified by `answer-pos`. This promise MUST 
resolve to the result the object returns. Messages sent to this promise MUST be 
delivered to the object when the promise resolves (unless the promise breaks). 
This promise should remain available until the [`op:gc-answer`](#opgc-answer) 
message is received. 

If the `answer-pos` is false, then promise pipelining is not used and the 
result of the message does not need to be held.
...
This is used to listen to a promise. This may be done in order to get notified when
the promise is:
...
`resolver-desc` MUST be a `desc:import` that is being imported. This will be
invoked when the promise condition is met.

If two `op:listen` messages are sent, one in shortening-only and one in 
settling-only mode, they MAY use the same `resolver-desc` since only one of 
the two listeners will be triggered.

`listen-mode` MUST be one of the following integers:

- 1: shortening-only
- 2: settling-only
- 3: shortening-or-settling

A shortening listener MUST be fulfilled if the promise is eventually fulfilled
with a promise on another peer (which could be the peer sending the `op:listen`).

A settling listener MUST be fulfilled if the promise is fulfilled with a
settled value or broken with an error.

An `op:listen` in shortening-only or shortening-or-settling mode MUST be sent 
after all [`op:deliver`](#opdeliver), [`op:index`](#opindex), 
[`op:untag`](#opuntag), and [`op:get`](#opget) messages are sent AND after any 
remote promise is received as an argument in [`op:deliver`](#opdeliver) args.

If the initial `op:listen` is sent in shortening-only mode, a later 
`op:listen` can be sent in settling-only mode to request the settled result of 
the promise. 

A peer MUST NOT add more than one shortening and one settling listener across
the wire, whether that be through a single `op:listen` in shortening-or-settling
mode or two `op:listen` messages where one is in shortening-only mode and the
other is in settling-only mode.

### Receiving

When receiving this message, providing `to-desc` matches a promise exported to
this session, a mechanism MUST be set up to fulfill or break the provided
resolver when the matching `listen-mode` condition is met by the `listen-desc` 
object.

A settling-only listener MUST be discarded without activation if promise 
shortening occurs.

An `op:listen` request MUST NOT be notified when the promise is fulfilled 
with another promise on the same peer until that promise has been fulfilled 
to meet the `listen-mode` condition, in which case the `op:listen` request is 
informed of that result.
...
The `op:get` MUST be followed up by an [`op:listen`](#oplisten) in either 
shortening-only or shortening-or-settling mode to request notification of any 
promise shortening and, in the shortening-or-settling case, the settling of 
the promise.
...
The `op:index` MUST be followed up by an [`op:listen`](#oplisten) in either 
shortening-only or shortening-or-settling mode to request notification of any 
promise shortening and, in the shortening-or-settling case, the settling of 
the promise.
...
The `op:untag` MUST be followed up by an [`op:listen`](#oplisten) in either 
shortening-only or shortening-or-settling mode to request notification of any 
promise shortening and, in the shortening-or-settling case, the settling of 
the promise.
