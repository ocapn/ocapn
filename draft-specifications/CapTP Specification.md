...
[`op:deliver`](#opdeliver) message containing a reference to a [resolver
object](#promise-and-resolver-objects). Otherwise, it sends a
[`op:deliver-only`](#opdeliver-only) message. Messages can also be "pipelined"
to promises generated by message sends with `op:deliver`, eliminating the need
for unnecessary round-trips via [Promise Pipelining](#promise-pipelining).
...
represent an eventual value which is initially unknown, and are said to be
pending until they become settled by either being fulfilled with a value or
broken with an error.

Promises are often created for representing the eventual response to an
[`op:deliver`](#opdeliver) message, and can be listened to with the
[`op:listen`](#oplisten) operation. They can be chained together in what is
called [Promise Pipelining](#promise-pipelining), whereby messages are sent
directly to a promise as a proxy for its eventual fulfillment value. Upon
fulfillment, such messages are relayed to that value. But if the promise breaks
or fulfills to a non-[Reference][Model-Reference] value, the messages cannot be
delivered and any promises representing their results should break.
...
Each promise is created with an associated resolver object that holds its
settlement powers and invokes them when notified by an appropriate message
(usually triggered remotely after providing a reference to the resolver object
in an [`op:deliver`](#opdeliver) or [`op:listen`](#oplisten) message).

The behavor of the two messages on the resolver object are as follows:

- `fulfill`: Provide the promise with its fulfillment value (success case)
- `break`: Break the promise (usually due to an error)

### `fulfill`

This method takes exactly one argument, the fulfillment value for the promise.
That value should be relayed to any listeners of the promise by sending each of
them an [`op:deliver-only`](#opdeliver-only) message to invoke their `fulfill`
method as well.

### `break`

This method takes exactly one argument, the reason for breakage (usually an
[Error][Model-Error]). That value should be relayed to any listeners of the
promise by sending each of them an [`op:deliver-only`](#opdeliver-only) message
to invoke their `break` method as well.
...
2. A non-negative integer gift ID that has not been used for a previous deposit
   by the Gifter in the **Gifter-Exporter** session.
...
1. Verify validity, including uniqueness of (Gifter, `gift-id`).
2. Associate the object being shared with (Gifter, `gift-id`).
...
2. Identify the **Gifter-Exporter** session from the `desc:handoff-give`. Use it
   to verify that (Gifter, `gift-id`) has not already been withdrawn and to
   verify the encapsulating `desc:sig-envelope`.
3. Validate the `desc:handoff-receive` within the **Exporter-Receiver** session
   (including uniqueness of (Receiver, `handoff-count`)) and its encapsulating
   `desc:sig-envelope` (note: the signature must correspond with `receiver-key`
   from the `desc:handoff-give`).
4. Record both (Receiver, `handoff-count`) and withdrawal of
   (Gifter, `gift-id`).
...
1. A `gift-id` that is a non-negative integer which has not been used for a
   previous deposit by the Gifter in this CapTP session.
...
5.  `gift-id` This is the gift ID that is generated by the Gifter that the
    gift will be deposited at. This `gift-id` MUST NOT have been used for a
    previous deposit by the Gifter in the **Gifter-Exporter** session.
