...
The following names are ***not*** reserved even though they have pre-existing meaning in Javascript. We do not need to reserve them because their expected contents and use are compatible enough with the ocapn rules for non-reserved error fields. Even though we do not separately standardize them, other peers that interoperate with Javascript may wish to be aware of their meaning in Javascript
  ...
  - `cause` A prior error that caused this one. For example, a high-level error might use `cause` to expose the proximate error from an internal library call in the hopes of facilitating root cause diagnosis.
  ...
  - `error` Introduced by Javascript's `SuppressedError` to represent an error that happens during handling of another error but is not _caused_ by it.
  - `suppressed` Introduced by Javascript's `SuppressedError` to represent an error for which handling encounters another error that is not necessarily related

  The round-tripping requirement is that ocapn errors must preserve all revealed information (with the possible exception of `message`). If a peer does not round trip all this information, it is not a conformant ocapn implementation. This round tripping requirement applies across language bindings, so an error containing information expected by language X but not Y must still round trip through Y. This resembles the round tripping constraints on `Tagged`. For example, a Javascript language binding built before the introduction of `FooError` must still round trip the `name: FooError` field so more recent peers can communicate FooErrors through old intermediaries without loss.
...
Some of that information, like the call stack, is important for diagnosing problems, and so should be locally associated with the error to enable selective disclosure. Someone who receives an error from a peer that trusts them to see this hidden information should be able to look it up, given helpful participation of the sending peer.
